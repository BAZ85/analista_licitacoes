...
def analisar():
    ...
    try:
        ...
        return jsonify({
            ...
            'download_json': f'{base}/download/{session_id}/json',
            'download_xlsx': f'{base}/download/{session_id}/xlsx'
        })
    
    except Exception as e:
        return jsonify({'status': 'erro', 'mensagem': str(e)}), 500

@app.route('/download/<session_id>/json', methods=['GET'])
def download_json(session_id):
    path = os.path.join(BASE_TMP_DIR, session_id, 'resultado_analise.json')
    return send_and_delete(path, 'resultado_analise.json')

@app.route('/download/<session_id>/xlsx', methods=['GET'])
def download_xlsx(session_id):
    path = os.path.join(BASE_TMP_DIR, session_id, 'resultado_analise.xlsx')
